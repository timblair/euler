...
].collect { |r| r.collect { |i| i.to_i }}  # intify everything

p = 0
m.each_index do |r|
  m[r].each_index do |c|
    p = [p, m[r][c..c+3].multiply].max if (c <= 16) # right
    p = [p, [m[r][c], m[r+1][c], m[r+2][c], m[r+3][c]].multiply].max if (r <= 16) # down
    p = [p, [m[r][c], m[r+1][c+1], m[r+2][c+2], m[r+3][c+3]].multiply].max if (c <= 16 && r <= 16) # right and down
    p = [p, [m[r][c], m[r-1][c+1], m[r-2][c+2], m[r-3][c+3]].multiply].max if (c <= 16 && r >= 3) # right and up
  end
end
